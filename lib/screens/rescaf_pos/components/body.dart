...
class ResCafPosMainBody extends StatefulWidget {

  const ResCafPosMainBody({Key? key}) : super(key: key);

  @override
  _ResCafPosMainBody createState() => _ResCafPosMainBody();

}

class _ResCafPosMainBody extends State<ResCafPosMainBody> with AfterLayoutMixin<ResCafPosMainBody> {
...
void initState() {
                      ...
                      child: ConstrainedBox(
                        ...
                        child: Row(
                          children: [
                              ...
                              SizedBox(width: getProportionateScreenWidth(defFormFieldEdges20)),
                              FuncButton100(text: lblCount, state: btnStateResCafCount,
                                press: () {
                                  if (btnStateResCafCount) {
                            ...
                            children: [
                              Column(
                                children: [
                                  ...
                                  FuncButton100(
                                    state: btnStateDelivered,
                                    text: txtResCafDeliItem,
                                    press: () {
                                      if (btnStateDelivered) {
                                        deliverItem();
                                      }
                                    },
                                  ),
                              ...
                              Column(
                                children: [
                                  SizedBox(height: getProportionateScreenHeight(defFormFieldEdges10)),
                                  FuncButton100(
                                    state: btnStateResCafPosChkout,
                                    text: txtResCafChkoutOrd,
                                    press: () {
                                      if (btnStateResCafPosChkout) {
                                        checkoutOrder();
                                      }
                                    },
                                  ),
                              ...
                              Column(
                                children: [
                                  SizedBox(height: getProportionateScreenHeight(defFormFieldEdges10)),
                                  FuncButton100(
                                    state: btnStateResCafPosPaid,
                                    text: txtResCafPaidOrd,
                                    press: () {
                                      if (btnStateResCafPosPaid) {
                                        setpaidOrder();
                                      }
                                    },
                                  ),
                              ...
                              Column(
                                ...
                                children: [
                                  const Text(" "),
                                  SizedBox(height: getProportionateScreenHeight(defFormFieldEdges10)),
                                  (ordspaidtottal>0)? Text("Tot: " + rescafCashHdrTot) : const Text(""),
                                  SizedBox(height: getProportionateScreenHeight(defFormFieldEdges01)),
                                  (ordspaidtax>0)? Text("Tax: " + rescafCashHdrTax) : const Text(""),
                                ],
                              ),
                            ],
                          ),
  ...
  void afterFirstLayout(BuildContext context) {
    ...
    setState(() {
      ...
      timerPaidOrder = Timer.periodic(const Duration(seconds: 60), (Timer t) => getPaidedOrders());
    });
  }
  ...
  syncPaidOrder(RescafOrder fromOrd, int ordidx) {
    setState(() {
      strpaidorders[ordidx].id=fromOrd.id;
      strpaidorders[ordidx].storeid=fromOrd.storeid;
      strpaidorders[ordidx].storename=fromOrd.storename;
      strpaidorders[ordidx].detail=fromOrd.detail;
      strpaidorders[ordidx].sessid=fromOrd.sessid;
      strpaidorders[ordidx].posid=fromOrd.posid;
      strpaidorders[ordidx].frontid=fromOrd.frontid;
      strpaidorders[ordidx].kitchenid=fromOrd.kitchenid;
      strpaidorders[ordidx].invid=fromOrd.invid;
      strpaidorders[ordidx].secposid=fromOrd.secposid;
      strpaidorders[ordidx].secfrontid=fromOrd.secfrontid;
      strpaidorders[ordidx].seckitid=fromOrd.seckitid;
      strpaidorders[ordidx].secinvid=fromOrd.secinvid;
      strpaidorders[ordidx].rescaf=fromOrd.rescaf;
      strpaidorders[ordidx].rescaftab=fromOrd.rescaftab;
      strpaidorders[ordidx].rescaftake=fromOrd.rescaftake;
      strpaidorders[ordidx].itemcount=fromOrd.itemcount;
      strpaidorders[ordidx].subtotal=fromOrd.subtotal;
      strpaidorders[ordidx].tax=fromOrd.tax;
      strpaidorders[ordidx].total=fromOrd.total;
      strpaidorders[ordidx].cash=fromOrd.cash;
      strpaidorders[ordidx].cashchg=fromOrd.cashchg;
      strpaidorders[ordidx].shipfee=fromOrd.shipfee;
      strpaidorders[ordidx].status=fromOrd.status;
      strpaidorders[ordidx].rescafcheckout=fromOrd.rescafcheckout;
      strpaidorders[ordidx].rescafcheckoutlog=fromOrd.rescafcheckoutlog;
      strpaidorders[ordidx].rescafpaid=fromOrd.rescafpaid;
      strpaidorders[ordidx].rescafpaidlog=fromOrd.rescafpaidlog;
      strpaidorders[ordidx].shipped=fromOrd.shipped;
    });
  }

  getPaidedOrders() {
    var postdata = {
        "qrymode": 'all_not_counted',
        "mobapp" : mobAppVal,
        "store_id" : "${thestore['id']}",
        "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
      };
    ...
    http.post(
        ...
        headers: posthdr
          ...
          if (result.statusCode == 200) {
            var resary = jsonDecode(result.body);
            setState(() {
              List<RescafOrder> tmpstrpaidorders = [];
              resary['data'].asMap().forEach((i, v) {
                RescafOrder tmpord = RescafOrder.fromJsonDet(v);
                tmpstrpaidorders.add(tmpord);
                if (strpaidorders.where((element) => element.id == tmpord.id).isNotEmpty) {
                  syncPaidOrder(tmpord, strpaidorders.indexWhere((element) => element.id == tmpord.id));
                } else {
                  strpaidorders.add(tmpord);
                }
              });
              strpaidorders.removeWhere((element) => (tmpstrpaidorders.where((bkelement) => bkelement.id == element.id).isEmpty));
              tmpstrpaidorders = [];

              ordspaidtottal = 0.0;
              ordspaidtax = 0.0;
              strpaidorders.asMap().forEach((pok, opv) {
                ordspaidtottal += opv.total;
                ordspaidtax += opv.tax;
              });
              rescafCashHdrTot = posCcy.format(ordspaidtottal) + ' ' + thestore['currency'];
              rescafCashHdrTax = posCcy.format(ordspaidtax) + ' ' + thestore['currency'];
              if (strpaidorders.isNotEmpty && ordspaidtottal>0) {
                btnStateResCafCount = true;
              } else {
                btnStateResCafCount = false;
              }
              // logMessage(strtabordprods.toString());
            });
          } else {
        ...
        children: <Widget>[
          ...
          ButtonTheme( // make buttons use the appropriate styles for cards
              child: ButtonBar(
                children: <Widget>[
                  TextButton(
                    child: Text(txtResCafNewOrder),
                    onPressed: () {
                      dbAddOrder(index);
                    },
                  ),
                  TextButton(
                    child: Text(txtResCafAddItem),
                    onPressed: () {
                      openAddItem(index);
                    },
                  ),
                  TextButton(
                    child: Text(txtResCafChgItem),
                    onPressed: () {
                      openChangeItem(index);
                    },
                  ),
                ]
              ),
  ...
  ListView _tabAllItemsListView() {
    return ListView.builder(
          ...
          shrinkWrap: true,
          padding: EdgeInsets.all(getProportionateScreenHeight(3)),
          itemCount: strallposordprods.length,
          itemBuilder: (context, index) {
            String currency = thestore['currency'];
            var detdata = strallposordprods[index];
  ...
  openChangeItem(tabid)  {
    setState(() {
      rescafcurtableid = tabid;
      rescafadditemmode = 'change';
      if (strtabs[tabid].orders.isNotEmpty && strtabs[tabid].orders[rescafcurorderidx].detail.isNotEmpty && strtabs[tabid].orders[rescafcurorderidx].detail[rescafcurorddetidx].id == rescafcurorddetid) {
        showItemAddDialog(context);
      }
    });
  }
  ...
  deliverItem() {
    var postdata = {
        ...
        "qrymode" : "frontdelivered",
        "secfront_id" : "${struser['id']}",
        "secfront_status" : "delivered",
        "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
      };
    var posthdr = {
        'Accept': 'application/json',
  ...
  checkoutOrder() {
      if (rescafcurtableid != 9999 && strtabs[rescafcurtableid].orders[rescafcurorderidx].detail.isNotEmpty && (strtabs[rescafcurtableid].orders[rescafcurorderidx].detail.where((element) => element.delivered==0)).isEmpty) {
        RescafOrder pickedorder = strtabs[rescafcurtableid].orders[rescafcurorderidx];
        cashPrintHdrTxt = '';
        ...
        curtkttotal = 0;
        curtkttax = 0;
        pickedorder.detail.asMap().forEach((di, dv) {
          cashPrintBodyTxt = cashPrintBodyTxt + dv.prdname+': x ' + dv.quantity.toString() + '\n';
          cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdyPrice + posCcy.format(dv.price) + ' ' + thestore['currency'] + '   ';
          cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdyTax + posCcy.format(dv.tax) + ' ' + thestore['currency'] + '\n';
          cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdySub + posCcy.format(dv.subtotal) + ' ' + thestore['currency'] + '\n\n';
        });
        cashPrintBodyTxt = cashPrintBodyTxt + '____________________________________\n';
        cashPrintFooterTxt = '';
        cashPrintFooterTxt = cashPrintFooterTxt + txtCashPrnBdyTotal + posCcy.format(pickedorder.total) + ' ' + thestore['currency'] + '\n';
        cashPrintFooterTxt = cashPrintFooterTxt + txtCashPrnBdyCash + posCcy.format(pickedorder.total) + ' ' + thestore['currency'] + '\n';
        // cashPrintFooterTxt = cashPrintFooterTxt + txtCashPrnBdyChange + posCcy.format(0.0) + ' ' + thestore['currency'] + '\n';
        ...
        var postdata = {
            ...
            "qrymode" : "checkout",
            "pos_id" : "${struser['id']}",
            "secpos_id" : "${struser['id']}",
            "rescaf_checkout" : "1",
            "rescafcheckout_log" : dateDbFormat.format(DateTime.now()),
            "status" : "Checked out at "+dateDbFormat.format(DateTime.now()),
            "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
          };
        var posthdr = {
            'Accept': 'application/json',
  ...
  setpaidOrder() {
      if (rescafcurtableid != 9999 && strtabs[rescafcurtableid].orders[rescafcurorderidx].detail.isNotEmpty && (strtabs[rescafcurtableid].orders[rescafcurorderidx].detail.where((element) => element.delivered==0)).isEmpty) {
        ...
        var postdata = {
            ...
            "secpos_id" : "${struser['id']}",
            "rescaf_paid" : "1",
            "rescafpaid_log" : dateDbFormat.format(DateTime.now()),
            "status" : "Marked as Paid order at "+dateDbFormat.format(DateTime.now()),
            "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
          };
        var posthdr = {
            'Accept': 'application/json',
  ...
  showCountDialog(BuildContext context) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return StatefulBuilder(
            builder: (context, setState) {
              return AlertDialog(
                title: Text(txtAlertCountResCaf),
                actions: <Widget>[
                  FuncButton100(text: lblCancel, state: btnStateResCafCountCancel,
                    press: () {
                        hideCountDialog(context);
                        setState(() {});
                    },
                  ),
                  FuncButton100(text: lblCount, state: btnStateResCafCountConfirm,
                    press: () {
                        countOrders();
                    },
                  ),
  ...
  countOrders() {
    strpaidorders.sort((a, b) => a.detail.first.created.compareTo(b.detail.first.created));
    cashPrintHdrTxt = '';
    cashPrintHdrTxt = cashPrintHdrTxt + txtCashPrnHdrStore + thestore['store_name']+'\n';
    cashPrintHdrTxt = cashPrintHdrTxt + txtCashPrnHdrPos + struser['usr_name']+ '\n';
    cashPrintHdrTxt = cashPrintHdrTxt + thestore['store_addr'] + '\n';
    ...
    double cursesstax = 0;
    int cursessords = 0;
    String cursessordids = '';
    strpaidorders.asMap().forEach((di, dv) {
      cursesstotal += dv.total;
      cursesssubtotal += dv.subtotal;
      cursesstax += dv.tax;
      cursessordids += ' ' + dv.id.toString();
      cursessords++;
    });
    cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdyNOfOrd + cursessords.toString() + '\n';
    ...
    cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdyStartTime + DateFormat.yMd().add_jms().format(strpaidorders.first.detail.first.created) + '\n';
    cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdyTransferTime + DateFormat.yMd().add_jms().format(DateTime.now()) + '\n';
    cashPrintBodyTxt = cashPrintBodyTxt + '____________________________________\n';
    cashPrintBodyTxt = cashPrintBodyTxt + txtCashPrnBdySignToTran + '\n\n\n\n';
    cashPrintBodyTxt = cashPrintBodyTxt + struser['name'] + ' [' + struser['usr_name'] + ']' + '\n\n\n';
    ...
    var postdata = {
        ...
        "sess_id" : "${possess['id']}",
        "total" : "$cursesstotal",
        "subtotal" : "$cursesssubtotal",
        "tax" : "$cursesstax",
        "order_count" : "$cursessords",
        "orders" : cursessordids.trim(),
        "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
      };
    var posthdr = {
        'Accept': 'application/json',
