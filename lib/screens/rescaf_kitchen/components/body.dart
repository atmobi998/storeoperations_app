...
import 'package:storeoperations_app/screens/rescaf_kitchen/components/product_list.dart';

class ResCafKitchenMainBody extends StatefulWidget {

  const ResCafKitchenMainBody({Key? key}) : super(key: key);

  @override
  _ResCafKitchenMainBody createState() => _ResCafKitchenMainBody();

}

class _ResCafKitchenMainBody extends State<ResCafKitchenMainBody> with AfterLayoutMixin<ResCafKitchenMainBody> {
...
void initState() {
                      ...
                      child: ConstrainedBox(
                        ...
                        child: Row(
                          children: [
                              ...
                              Column(
                                children: <Widget>[
                                  Text(txtResCafKitchenTitle,style: TextStyle(fontSize: getProportionateScreenWidth(18),color: kPrimaryColor,fontWeight: FontWeight.bold)),
                            ...
                            children: [
                              Column(
                                children: [
                                  ...
                                  FuncButton100(
                                    state: btnStateKitchenAccept,
                                    text: txtResCafAcceptItem,
                                    press: () {
                                      if (btnStateKitchenAccept) {
                                        acceptItem();
                                      }
                                    },
                                  ),
                              ...
                              Column(
                                children: [
                                  SizedBox(height: getProportionateScreenHeight(defFormFieldEdges10)),
                                  FuncButton100(
                                    state: btnStateKitchenDone,
                                    text: txtResCafDoneItem,
                                    press: () {
                                      if (btnStateKitchenDone) {
                                        doneItem();
                                      }
                                    },
                                  ),
  ...
  ListView _tabAllItemsListView() {
    return ListView.builder(
          ...
          shrinkWrap: true,
          padding: EdgeInsets.all(getProportionateScreenHeight(3)),
          itemCount: strallkitordprods.length,
          itemBuilder: (context, index) {
            String currency = thestore['currency'];
            var detdata = strallkitordprods[index];
  ...
  acceptItem() {
    var postdata = {
        ...
        "qrymode" : "kitchenaccept",
        "kitchen_id" : "${struser['id']}",
        "kitchen_status" : "kitchenaccepted",
        "created" : dateDbFormat.format(DateTime.now()), "time" : dateDbFormat.format(DateTime.now()),
      };
    var posthdr = {
        'Accept': 'application/json',
